...
export const createProduct = async (req, res) => {
	try {
		const imagesNames = [];
		const { productName, shortDescription, longDescription, quantity, releaseYear, brand, category, company, os, price } = req.body;
...
export const deleteProduct = async (req, res) => {
	//only make the status unavailable!
	const id = req.body.id;
	try {
		await Product.findByIdAndUpdate(id, { status: 'unavailable' });
...
export const checkIfExist = async (req, res) => {
	try {
		...
		if (!checkItem) {
			...
			{
				ProductInfoLogger.log('info', 'product is not exist. status code: 200');
				res.send([]);
			}
		}
	} catch (error) {
